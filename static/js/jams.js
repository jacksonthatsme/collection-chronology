...
function timeFromSecs(seconds) {
    var minute = Math.floor(((seconds/3600)%1)*60);
    var second = Math.round(((seconds/60)%1)*60);
    if (second < 10 ){
      second = "0"+second;
    }
...
function buildPlaylists(data) {
  for (var i=0; i<data.length; i++) {
    ...
    var playlist = data[i];
    //make li.playlist
    var playlistLi = $('<li>').addClass('playlist').attr('id', playlist.name.replace(/\s/g, '').toLowerCase());
...
function buildIndex(data){
  for (var i=0; i<data.length; i++) {
    ...
    //define index li
    var indexLi = $('<li>');
    var linkDown = $('<a>').attr('href', '#' + playlist.name.replace(/\s/g, '').toLowerCase());
